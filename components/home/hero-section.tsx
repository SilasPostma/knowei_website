...
export default function HeroSection() {
  ...
  // MOBILE TRANSFORMS
  const introTextOpacityMB = useTransform(scrollYProgress, [0.02, 0.1], [1, 0]);
  const bgOpacityMB = useTransform(
    scrollYProgress,
    [0.05, 0.12, 0.2, 0.28],
    [0, 1, 1, 0]
  );
  const w10OpacityMB = useTransform(scrollYProgress, [0.18, 0.28], [1, 0]);
  ...
  const l10YMB = useTransform(scrollYProgress, [0.5, 0.75], ["0%", "-150%"]);
  ...
  return (
    ...
    >
      ...
      <div
        className={`${
          isMobile ? "relative" : "sticky top-0 h-screen"
      ...
      >
        ...
        >
          {/* Logos Component */}
          {/* Added a sticky-top wrapper for mobile logos so they stay visible during text scroll */}
          <div
            className={`flex ${
              isMobile
          ...
          >
            ...
            {/* L10 (Icon Logo) - Right on Desktop, Top on Mobile */}
            <motion.div
              style={{ y: l10Y }}
              className={`flex-1 flex ${
          ...
          {/* Changed from absolute to relative on mobile to prevent clipping */}
          <motion.div
